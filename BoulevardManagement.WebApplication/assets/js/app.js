...
function Close(e) {
	...
	if (PressedRecordButton == "Remarks") {
		myFileRemarks = null;
	}
	if (PressedRecordButton == "CurrentComplaint") {
		myFileCurrentComplaint = null;
	}
	if (PressedRecordButton == "RiskAssessment") {
		MyFileRiskAssessment = null;
	}
	if (PressedRecordButton == "CaseSummary") {
		MyFileCaseSummary = null;
	}
	if (PressedRecordButton == "TherapeuticPlans") {
		MyFileTherapeuticPlan = null;
	}
	if (PressedRecordButton == "Notes") {
		myFileNotes = null;
	}
	if (PressedRecordButton == "ConsultationNote") {
			...
			*/
			...
			$(document).ready(function () {
				recorder.onComplete = function (recorder, blob) {
					...
					if (PressedRecordButton =="Remarks") {
						myFileRemarks = myFile;
									
					}
					if (PressedRecordButton == "CurrentComplaint") {
						myFileCurrentComplaint = myFile;
					}
					if (PressedRecordButton == "RiskAssessment") {
						MyFileRiskAssessment = myFile;
					}
					if (PressedRecordButton == "CaseSummary") {
						MyFileCaseSummary = myFile;
					}
					if (PressedRecordButton == "TherapeuticPlans") {
						MyFileTherapeuticPlan = myFile;
					}
					debugger;
					if (PressedRecordButton == "Notes") {
						myFileNotes = myFile;
					...
					if (!IsModalClosePressed) {
						if (!isStop) {
							if (!IsReplayButton) {
								...
								if (IsStickyNoteSection!=undefined) {
...
function stopRecording(e) {
	...
	if (btnId == "StopRecordBtn") {
		...
		if (PressedRecordButton == "Remarks") {
			$('#Remarkrecord').html('');
			$('#Remarkrecord').append(recordingsList.innerHTML);
			$('#RemarkrecordTrash').show();
			$('#RemarksVoiceRecorder').hide();

		}
		if (PressedRecordButton == "CurrentComplaint") {
			$('#CurrentComplaintVoicerecord').html('');
			$('#CurrentComplaintVoicerecord').append(recordingsList.innerHTML);
			$('#CurrentComplaintTrash').show();
			$('#CurrentComplaintRecorder').hide();
		}
		if (PressedRecordButton == "RiskAssessment") {
			$('#RiskAssessmentVoicerecord').html('');
			$('#RiskAssessmentVoicerecord').append(recordingsList.innerHTML);
			$('#RiskAssessmentTrash').show();
			$('#RiskAssessmentRecorder').hide();
		}
		if (PressedRecordButton == "CaseSummary") {
			$('#CaseSummaryVoicerecord').html('');
			$('#CaseSummaryVoicerecord').append(recordingsList.innerHTML);
			$('#CaseSummaryTrash').show();
			$('#CaseSummaryRecorder').hide();
		}
		if (PressedRecordButton == "TherapeuticPlans") {
			$('#TherapeuticPlanVoicerecord').html('');
			$('#TherapeuticPlanVoicerecord').append(recordingsList.innerHTML);
			$('#TherapeuticPlanTrash').show();
			$('#TherapeuticPlanRecorder').hide();
		}
		if (PressedRecordButton == "Notes") {
			debugger;
			$('#Notesrecord').html('');
			$('#Notesrecord').append(recordingsList.innerHTML);
			$('#NotesrecordTrash').show();
			$('#NotesVoiceRecorder').hide();
		}

		if (PressedRecordButton == "ConsultationNote") {
			$('#ConsultationNoterecord').html('');
			$('#ConsultationNoterecord').append(recordingsList.innerHTML);
			$('#ConsultationNoterecordTrash').show();
			$('#ConsultationNoteVoiceRecorder').hide();
		}
	}
