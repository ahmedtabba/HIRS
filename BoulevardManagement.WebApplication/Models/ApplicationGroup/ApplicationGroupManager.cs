...
namespace BoulevardManagement.WebApplication.Models
{
    public class ApplicationGroupManager : IDisposable
    {
        ...
        public IdentityResult SetGroupRoles(string groupId, params string[] roleNames)
        {
            ...
            var thisGroup = _db.ApplicationGroups.Include(c => c.ApplicationRoles).Include(c => c.ApplicationUsers).Single(c => c.Id == groupId);
        ...
        public IEnumerable<ApplicationGroup> GetUserGroups(string userId)
        {
            ...
            //                  where g.ApplicationUsers.Any(u => u.ApplicationUserId == userId)
            //                  select g).ToList();
            var userGroups = this.Groups.Include(c => c.ApplicationRoles).Include(c => c.ApplicationUsers).Where(c => c.ApplicationUsers.Any(d => d.ApplicationUserId == userId)).ToArray();
